...
class VisitRestControllerTests {
    ...
    @Test
    @WithMockUser(roles="OWNER_ADMIN")
    void testCreateVisitWithFutureDate() throws Exception {
        Visit newVisit = new Visit();
        newVisit.setPet(visits.get(0).getPet());
        newVisit.setDate(LocalDate.now().plusDays(7));
        newVisit.setDescription("scheduled checkup");
        ...
        this.mockMvc.perform(post("/api/visits")
            ...
            .andExpect(status().isCreated())
            .andExpect(jsonPath("$.description").value("scheduled checkup"));
    }
}
