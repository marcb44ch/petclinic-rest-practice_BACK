...
class PetRestControllerTests {
    ...
    /**
     * Valida l'actualització d'una mascota canviant el seu tipus (dog → cat).
     * Valor: Cobreix un escenari d'edició complexa on es modifica la relació
     * amb PetType, important per correccions d'errors de registre inicial.
     */
    @Test
    @WithMockUser(roles = "OWNER_ADMIN")
    void testUpdatePetWithModifiedType() throws Exception {
        PetDto existingPet = pets.get(0);
        given(this.clinicService.findPetById(3)).willReturn(petMapper.toPet(existingPet));
        
        PetTypeDto newType = new PetTypeDto();
        newType.id(1).name("cat");
        
        PetDto updatedPet = new PetDto();
        updatedPet.id(3)
            .name("Rosy")
            .birthDate(existingPet.getBirthDate())
            .type(newType);
    ...
    /**
     * Valida el comportament d'actualització amb ID inexistent.
     * Valor: Garanteix resposta 404 NOT FOUND per IDs invàlids, prevenint
     * actualitzacions accidentals i proporcionant validació de dades robusta.
     */
    @Test
    @WithMockUser(roles = "OWNER_ADMIN")
    void testUpdatePetWithInvalidId() throws Exception {
        given(this.clinicService.findPetById(777)).willReturn(null);
        ...
        this.mockMvc.perform(put("/api/pets/777")
            .content(updatedPetAsJSON)
            .accept(MediaType.APPLICATION_JSON_VALUE)
            .contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(status().isNotFound());
