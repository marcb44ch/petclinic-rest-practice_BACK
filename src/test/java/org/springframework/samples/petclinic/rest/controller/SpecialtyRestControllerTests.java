...
class SpecialtyRestControllerTests {
    ...
    @Test
    @WithMockUser(roles="VET_ADMIN")
    void testFindSpecialtyByName() throws Exception {
        given(this.clinicService.findAllSpecialties()).willReturn(specialties);
        ...
        this.mockMvc.perform(get("/api/specialties")
            ...
            .andExpect(status().isOk())
            .andExpect(content().contentType("application/json"))
            .andExpect(jsonPath("$[?(@.name == 'radiology')]").exists())
            .andExpect(jsonPath("$[?(@.name == 'surgery')]").exists())
            .andExpect(jsonPath("$[?(@.name == 'dentistry')]").exists());
    }

    @Test
    @WithMockUser(roles="VET_ADMIN")
    void testDeleteSpecialtyNotFoundAlternative() throws Exception {
        given(this.clinicService.findSpecialtyById(888)).willReturn(null);
        
        this.mockMvc.perform(delete("/api/specialties/888")
            .accept(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(status().isNotFound());
    }
}
